...
// exact same Go types cmd/bogoslav-mcp/server.go's mcp.AddTool
...
// types do: cmd/bogoslav-mcp is package main, and Go forbids
...
// internal/app request, building an output value from an
// internal/app result, and every other tool handler, stays in
// cmd/bogoslav-mcp: this package is data only.
//
...
// cmd/bogoslav-mcp/server.go's mcp.AddTool call works around this
// today by registering the get_classify_batch tool with an "any" output
// type parameter (see cmd/bogoslav-mcp/tool_get_classify_batch.go);
// internal/contracts does the same, and documents why (see its own
