...
// &jsonschema.ForOptions{} value cmd/bogoslav-mcp's mcp.AddTool
...
// library's own code path -- see internal/mcptool/schema_test.go,
...
// field of every one of internal/mcptool's *Input and *Output
// types: every from/to-shaped field there is a plain wire string,
// parsed into a domain.Date only inside cmd/bogoslav-mcp's
...
//
// What is included:
//   - the six MCP tool input types from internal/mcptool
...
//     internal/mcptool (FindMRsOutput, GetCommentsOutput,
//     SaveLabelsOutput, FilterCommentsOutput, GetStatsOutput);
//   - the four artifact document types from internal/artifact
...
//	cmd/bogoslav-mcp/tool_get_classify_batch.go registers that
...
// cmd/bogoslav-mcp, alongside the mcp.AddTool calls that use them --
...
// jsonschema.ForType on them. They now live in internal/mcptool,
// next to their matching input types and GetClassifyBatchOutput, for
// exactly that reason; cmd/bogoslav-mcp imports them from there, and
// this package can too.
//
// Regenerate with `make contracts`, or `go generate` from this
// package's directory.
package contracts
