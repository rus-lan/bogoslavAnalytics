...
Artifacts and cache: a command that produces a result writes it under
--artifacts-dir (default "./artifacts") in --format (json, yaml, text, or
html). That same file is also the cache for the next identical request:
json and yaml artifacts are looked up, by their normalized request, before
find-mrs or get-comments call GitLab; --refresh bypasses the cache and
always calls GitLab. text and html are write-only: neither round-trips
back into structured data, so neither is ever a cache hit and neither can
be passed to --from-artifact.
