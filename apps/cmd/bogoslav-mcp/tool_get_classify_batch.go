...
// server.go), not itself: Schema's own Go type
...
func newGetClassifyBatchRequest(in GetClassifyBatchInput) app.GetClassifyBatchRequest {
...
func (s *toolServer) getClassifyBatch(_ context.Context, _ *mcp.CallToolRequest, in GetClassifyBatchInput) (*mcp.CallToolResult, any, error) {
	result, err := app.GetClassifyBatch(newGetClassifyBatchRequest(in))
	if err != nil {
		return nil, nil, fmt.Errorf("get_classify_batch: %w", err)
	}

	out := GetClassifyBatchOutput{
